...
//   baseURL: '', // set per-request in interceptor so build-time Node doesn't freeze it to '/api'
...
export function getErrorMessage(
  ...
  if (obj && typeof obj === 'object' && !Array.isArray(obj)) {
    const msg =
      (obj as { error?: string; message?: string }).error ??
...
export const authApi = {
  register: (name: string, phone: string) =>
    api.post<{ data: { user: User; token: string } }>(
      '/auth/register',
      { name, phone }
    ),
...
export const promptsApi = {
  create: (
    ...
    api.post<{ data: PromptItem }>('/prompts', {
      categoryId,
      subCategoryId,
      prompt,
    }),
...
export const adminApi = {
  ...
  createCategory: (name: string) =>
    api.post<{ data: Category }>(
      '/admin/categories',
      { name }
    ),

  updateCategory: (id: string, name: string) =>
    api.patch<{ data: Category }>(
      `/admin/categories/${id}`,
      { name }
    ),
